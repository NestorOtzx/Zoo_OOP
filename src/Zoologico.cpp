...
void Zoologico::editarAlimento() {
    ...
    if (habitats.size() < 1)
    {
        std::cout<<"No hay animales para editar sus alimentos"<<std::endl;
        return;
    }
    ...
    if ((*habitat)->getCantidadAnimales() < 1)
    {
        std::cout<<"No hay animales en el habitat para editar sus alimentos"<<std::endl;
        return;
    }
    ...
    switch (tipoAlimento) {
        ...
        case 2:
            ...
            break;
        default:
            throw std::invalid_argument("Ese tipo de alimento no existe en el zoologico.");
            return;
    }

    //Confirmar si el animal puede consumir ese tipo de alimento
    if(animal->getTipoAlimento() == TipoAlimento::Omnivoro || animal->getTipoAlimento() == nuevoAlimento.getTipo())
    {
        ...
        std::cout << "No puedes alimentar a " << animal->getNombre() << " con un alimento " << Animal::tipoAlimentoAString(
                nuevoAlimento.getTipo()) << "." << std::endl;
        std::cout << animal->getNombre() << " es un animal " << Animal::tipoAlimentoAString(animal->getTipoAlimento()) << " y solo puede ser alimentado con ese tipo de comida." << std::endl;
    }
}
